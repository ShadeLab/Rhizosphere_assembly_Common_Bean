...
rplane_otu_table_high <- rplane_otu_table_w_abund %>% filter(taxa_sums.rplane_otu_table. > 10000) %>% arrange(desc(taxa_sums.rplane_otu_table.))
...
rplane_high_tax$Genus[11] <- "f__Comamonadaceae"
rplane_high_tax$Genus[14] <- "f__Comamonadaceae"
rplane_high_tax$Genus[18] <- "f__Comamonadaceae"
rplane_high_tax$Genus[24] <- "f__Comamonadaceae"
rplane_high_tax$Genus[26] <- "f__Comamonadaceae"
rplane_high_tax$Genus[27] <- "f__Comamonadaceae"
rplane_high_tax$Genus[28] <- "f__Comamonadaceae"
rplane_high_tax$Genus[30] <- "f__Comamonadaceae"
rplane_high_tax$Genus[36] <- "f__Comamonadaceae"
rplane_high_tax$Genus[37] <- "f__Comamonadaceae"
rplane_high_tax$Genus[46] <- "f__Comamonadaceae"
rplane_high_tax$Genus[50] <- "f__Comamonadaceae"

rplane_high_tax$Genus[12] <- "f__Methyloligellaceae"

rplane_high_tax$Genus[19] <- "o__Bacillales"
rplane_high_tax$Genus[55] <- "o__Bacillales"

rplane_high_tax$Genus[56] <- "f__Rhizobiaceae"
rplane_high_tax$Genus[47] <- "f__Rhizobiaceae"

rplane_high_tax$Genus[23] <- "f__Micrococcaceae"

rplane_high_tax$Genus[44] <- "f__Spirosomaceae"

rplane_high_tax$Genus[40] <- "f__Oxalobacteraceae"

rplane_high_tax$Genus[] == "g__Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium" 
# 9, 34, 54, 57 
rplane_high_tax$Genus[9] <- "g__Allo-Neo-Para-Rhizobium"
rplane_high_tax$Genus[34] <- "g__Allo-Neo-Para-Rhizobium"
rplane_high_tax$Genus[54] <- "g__Allo-Neo-Para-Rhizobium"
rplane_high_tax$Genus[57] <- "g__Allo-Neo-Para-Rhizobium"

family_list <- unique(c(rplane_high_tax$Family)) 
#21 families
genus_list <- unique(c(rplane_high_tax$Genus))
#33 genera
number_list <- c(1:33)

genus_numbers <- data.frame(genus_list, number_list) 
colnames(genus_numbers)[1] <- "Genus"
new_table <- left_join(rplane_high_tax, genus_numbers, by="Genus")
...
annotate_genus <- rowAnnotation(ASV_Tax = rplane_high_tax$number_list, col = list(ASV_Tax = c('1' = colors_34[1], '2' = colors_34[2], '3' = colors_34[3], '4' = colors_34[4], '5' = colors_34[5], '6' = colors_34[6], '7' = colors_34[7], '8' = colors_34[8], '9' = colors_34[9], '10' = colors_34[10], '11' = colors_34[11], '12' = colors_34[12], '13' = colors_34[13], '14' = colors_34[14], '15' = colors_34[15], '16' = colors_34[16], '17' = colors_34[17], '18' = colors_34[18], '19' = colors_34[19], '20' = colors_34[20], '21' = colors_34[21], '22' = colors_34[22], '23' = colors_34[23], '24' = colors_34[24], '25' = colors_34[25], '26' = colors_34[26], '27' = colors_34[27], '28' = colors_34[28], '29' = colors_34[29], '30' = colors_34[30], '31' = colors_34[31], '32' = colors_34[32], '33' = colors_34[33])),                                  
    annotation_legend_param = list(ASV_Tax = list(
                title = "ASV Taxonomy",
                at = number_list,
                labels = genus_list)))
...
rhizoplane_hm <- Heatmap(rplane_high_otu, name="ASV reads\nper sample", show_row_names = FALSE, column_split=rplane_high_meta$series, top_annotation = annotate, row_title = "ASV", right_annotation = annotate_genus, column_title = series_names, use_raster = TRUE, 
        raster_device = "png")
draw(rhizoplane_hm, heatmap_legend_side = "right")

pdf(file="/Users/Abby/OneDrive - Michigan State University/Rhizo_assembly/heatmap.pdf", width = 16, height = 9)
rhizoplane_hm <- Heatmap(rplane_high_otu, name="ASV reads\nper sample", show_row_names = FALSE, column_split=rplane_high_meta$series, top_annotation = annotate, row_title = "ASV", right_annotation = annotate_genus, column_title = series_names, column_names_gp = gpar(fontsize = 6))
draw(rhizoplane_hm, heatmap_legend_side = "right", annotation_legend_side = "right")
dev.off()
```
...
table(stage_indicator_ASVs$indic_stage)
...
rplane_ASV_table_filter <- rplane_otu_table_w_abund %>% filter(taxa_sums.rplane_otu_table. > 5) %>% arrange(desc(taxa_sums.rplane_otu_table.))
# 4,303 taxa
...
above.pred = sum(rplane_4_nm$predictions.freq > (rplane_4_nm$predictions.pred.upr), na.rm=TRUE)/nrow(rplane_4_nm)  # fraction of OTUs above prediction # 13.61 %
below.pred = sum(rplane_4_nm$predictions.freq < (rplane_4_nm$predictions.pred.lwr), na.rm=TRUE)/nrow(rplane_4_nm)  # fraction of OTUs below prediction # 4.32 %
...
ggsave(occ_abund_plot, filename="/Users/Abby/OneDrive - Michigan State University/Rhizo_assembly/occ_abund_plot_2.pdf", dpi=300, width=9, height=6, units="in")
...
unique(selected_indicator_taxa_abund$predictions.Genus)
unique(selected_indicator_taxa_abund$predictions.Family)
unique(selected_indicator_taxa_abund$predictions.Order)
...
#bar plot of selected indicators?
indic_barplot <- ggplot(data=selected_indicator_taxa_abund, aes(x=factor(indic_stage.y, levels=sampling_order_growth), fill=predictions.Class)) +
  geom_bar(position = "stack") +
  xlab("Indicator Taxa Growth Stage") + ylab("Selected Indicator Taxa Count") +
  scale_fill_manual(name="Indicator Taxa\nClass", breaks=waiver(), values=colors_34) +
  my_theme +
  guides(fill=guide_legend(ncol=5)) + theme(legend.position = "right")
indic_barplot

ggsave(indic_barplot, filename="/Users/Abby/OneDrive - Michigan State University/Rhizo_assembly/indicator_barplot.pdf", dpi=300, width=13, height=6, units="in")


  
#bar plot of selected indicators?
# indic_barplot <- ggplot(data=selected_indicator_taxa_abund, aes(x=indicator, y=mean_rel_abund, fill=predictions.Class)) +
